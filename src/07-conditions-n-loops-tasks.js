...
function doRectanglesOverlap(rect1, rect2) {
  const xOverlap = Math.max(
    0,
    Math.min(rect1.left + rect1.width, rect2.left + rect2.width) - Math.max(rect1.left, rect2.left),
  );
  const yOverlap = Math.max(
    0,
    Math.min(rect1.top + rect1.height, rect2.top + rect2.height) - Math.max(rect1.top, rect2.top),
  );
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (bracket === '[' || bracket === '(' || bracket === '{' || bracket === '<') {
      ...
      if (stack.length === 0 || stack[stack.length - 1] !== bracketsMap[bracket]) {
        return false;
      }
      stack.pop();
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  const separator = '/';
  const parts = pathes[0].split(separator);
  let commonPath = '';
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    for (let j = 1; j < pathes.length; j += 1) {
      const pathParts = pathes[j].split(separator);
      if (pathParts.length <= i || pathParts[i] !== currentPart) {
        isCommon = false;
        break;
      }
    }
    ...
    commonPath += currentPart + separator;
  }
...
function getMatrixProduct(m1, m2) {
  ...
  if (colsM1 !== rowsM2) {
    throw new Error('Invalid matrix dimensions: the number of columns in the first matrix must be equal to the number of rows in the second matrix.');
  }
